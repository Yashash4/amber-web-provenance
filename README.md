...
**Premium brands lose millions a year to gray-market diversion. Amber catches the cross-border price gap across markets and turns it into a margin figure a brand can act on, backed by signed evidence that can't be forged.**
...
| **Tamper, and it breaks (THE forge-proof)** | Edit one byte of the signed facts, the Merkle root changes, ed25519 fails: **RED, CHAIN OF CUSTODY BROKEN**; revert and it goes GREEN | `cd web && npm run demo` *(drives GREEN, RED, GREEN through the REAL verifier exit code)* |
| **The net-of-tax gap (the channel, not the tax)** | DE net **€150.42** (19% VAT) vs BE net **€139.67** (21% VAT), a **€10.75/unit** net-of-tax delta, DE dearer. Same GTIN `0195949689673`, both `PURCHASABLE` | `python -m amber.capture_cli` facts ship in [`samples/live_packet/facts.json`](./samples/live_packet/facts.json) |
...
Editing a single character of a signed fact changes its Merkle leaf, which changes the root, so the signature no longer covers it. The verifier names the broken node (`facts.json`) and exits non-zero. Revert the byte and it heals. **That is THE TAMPER PROOF.** The same `verify_packet` exit code drives the `npm run demo` GREEN, RED, GREEN.
...
| | |
|---|---|
| **One-command offline demo** | `cd web && npm run demo` (GREEN, RED, GREEN, the real verifier) · interactive UI: `npm run dev` then http://localhost:3000 |
...
| **Architecture diagram** | [`samples/architecture-diagram.png`](./samples/architecture-diagram.png) (plus mermaid below) |
| **Cover image** | [`samples/cover-image.png`](./samples/cover-image.png) |
| **Built on** | [Bright Data](https://brightdata.com/) residential network, the fetcher and geo-witness |
...
![Amber architecture: Bright Data residential capture, then deterministic Layer-1 floor, then sha256/Merkle/ed25519 signed provenance, then offline verify_packet; Layer-2 advisory layers (jury, memory, workflow) read the signed facts but are never signed; the geo_fanout contribute-back PR](./samples/architecture-diagram.png)
...
**The hard line (LOCK 4):** the signed bundle contains **only** Layer-1 deterministic facts plus raw captures. Every Layer-2 layer (jury, memory, workflow) *reads* the signed `facts.json` and writes its output as a **physically separate, UNSIGNED** sibling. An LLM never computes a number or asserts a fact into the signed record. (Verified: a packet still `verify_packet`s GREEN after every Layer-2 layer runs over it.)
...
`verify_packet` checks the signature against a trusted signer key supplied **out-of-band**, never solely the key bundled in the packet (a self-attesting packet proves only "signed by whoever signed it," which is nothing). Precedence:
...
| `classifyResponse` | Classifies a response (OK / transient / blocked / geo-refused) so a retry policy can act on the *class*, not a bare status. This is **the literal #104 fix**, in its own commit. | Pure function, table-driven tests. |
...
| **Cognee** | A self-hosted **temporal knowledge graph** (Gemini backend, one key) answers *"has this SKU shown a net-of-tax gap before, and is it persistent?"*, the leap from a one-off gap to a margin-leak signal. **Real captures only**; one capture is a BASELINE, never a faked chart. | Best Use of Agent Memory ($2.4k) |
| **TriggerWare.ai** | A signed delta crossing the brand's threshold **fires a trigger** (SQL-over-everything, confirmed live against `api.triggerware.com`), then a brand-protection **alert stating the signed FACT**, never a verdict. The event-driven close-the-loop. | Best Use of Automated Workflows ($300) |
...
|---|---|---|---|
| **Signed-provenance spine** | [`amber/packet.py`](./amber/packet.py), [`signer.py`](./amber/signer.py), [`merkle.py`](./amber/merkle.py), [`verifier.py`](./amber/verifier.py), [`cli.py`](./amber/cli.py) | `seal_packet` / `verify_packet` plus CLI. ed25519 signer/verifier reused from Reef; RFC 6962 Merkle ported Go to Python. | 42 |
...
- **Does NOT prove causation.** Geo-attribution here is **`EXIT_ONLY`**, not `CONFIRMED`: the packet shows the request left a DE/BE residential exit and the store served a country page, but it does **not** prove the store served that page *because of* the exit IP. The geo variable actually doing the work is the **ccTLD storefront** (`.de` vs `.be`), not the exit IP. We reserve the "served-because-of-country" causal claim for the `GEO_BLOCKED` path, which requires **≥2 causally-independent signals** and is **not** claimed for this price-delta packet.
...
- **"Dispatched," not "witnessed."** Captures are *dispatched* the same second (`dispatched_same_second=true`, a signed fact); residential **responses** physically land seconds apart (`same_second_batch=false`, reported honestly), so we never claim a witnessed same-second.
...
- **Provenance / `session_budget` PRs** (#132): the second and third upstream contributions, after #141.
...
**MIT**, see [LICENSE](./LICENSE). Only `cryptography` (Apache-2.0/BSD) is required at runtime. The `web/` demo is Apache-2.0. GPL/AGPL libraries (`pymerkle`, OpenWPM, browsertrix) are **cited, never imported**, so Amber ships clean.

---

## Citations & acknowledgments

- **Bright Data:** the residential network is Amber's fetcher and geo-witness; the contribute-back PR ([#141](https://github.com/brightdata/brightdata-mcp/pull/141)) closes brightdata-mcp #104. Amber claims no Bright Data endorsement.
...
- **AI/ML API · Cognee · TriggerWare.ai · Kiro:** the four honestly load-bearing partners.
