...
/**
 * THE BUSINESS NUMBER (supporting data panel): the signed EUR/unit delta
 * multiplied by a BUYER-SUPPLIED annual volume equals a EUR/yr
 * recoverable-margin figure. The volume is labeled an ASSUMPTION on screen
 * (never observed); the per-unit delta is the signed measurement. This is the
 * card that turns a forensic curiosity into a CRO-legible outcome: Amber signs
 * the per-unit delta, the brand supplies the volume.
 */
export function BusinessNumber({ view }: { view: PacketView }) {
  ...
  return (
    <div className="panel-amber flex flex-col p-4 sm:p-5">
      <PanelHead
      ...
      {/* The equation. */}
      <div className="mt-4 flex flex-wrap items-center gap-x-3 gap-y-2 text-white/80">
        <Token value={`EUR ${bi.netDeltaPerUnit}`} label="signed delta / unit" accent />
        <span className="text-2xl font-light text-white/40">x</span>
        <Token value={groupThousands(bi.annualDivertedUnits)} label="units / yr (assumption)" />
        <span className="text-2xl font-light text-white/40">=</span>
      </div>

      {/* The headline figure. */}
      <div className="mt-4 rounded-xl border border-amber/40 bg-[#0a0a0c]/60 px-5 py-4 text-center">
      ...
      <div className="mt-4 flex items-start gap-2 rounded-lg border border-white/10 bg-black/30 px-4 py-2.5 text-[11px] leading-relaxed text-white/50">
...
function Token({ value, label, accent }: { value: string; label: string; accent?: boolean }) {
  return (
    <div
    ...
    >
      <div className={`text-xl font-black tabular-nums ${accent ? "text-amber" : "text-white/85"}`}>
        {value}
      </div>
